...
export function buildPaginator<Entity extends ObjectLiteral>(
  ...
  const {
    ...
    paginationKeys = ['id' as Extract<keyof Entity, string>],
  } = options;
...
export default class Paginator<Entity extends ObjectLiteral> {
  ...
  private getEntityPropertyType(key: string): string {
    const meta = Reflect.getMetadata(
      'design:type',
      this.entity.prototype as object,
      key,
    ) as { name: string } | undefined;
    return meta?.name.toLowerCase() ?? 'unknown';
  }
...
function encodeByType(type: string, value: unknown): string | null {
  ...
  switch (type) {
    ...
    case 'object': {
      ...
      /**
       * Support for branded id's (e.g. Uuid extending String) that expose toString().
       */
  ...
  throw new Error(`unknown type in cursor: [${type}]`);
}
